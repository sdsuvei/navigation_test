...
PedestrianDetector::PedestrianDetector(std::string strPedModel){
	...
	if(pedModel.filterDim[0] == 0) { // The ACF-type detector
		...
		imagePyramid.shrinkLastStage = imagePyramid.shrinkFirstStage*2;
	}
	//shrink = 4;
...
void PedestrianDetector::makeKernels() {
	...
	for(int iiChKernel = 0; iiChKernel < nKernels; iiChKernel++) {// Over 4 kernels
		for(int iChannel = 0; iChannel < nChannels; iChannel++) { // Over 10 scales
			...
			flip(tmpKernel,pedModel.kernel[iChannel][iiChKernel],-1);
			//pedModel.kernel[iChannel][iiChKernel] = tmpKernel;
		}
	}

//	int rearrangeKernels[] = {2,1,0,3,9,8,7,6,5,4}; // lUV 3 Mag1 Hist6
//
//	for(int iChannel = 0; iChannel < nChannels; iChannel++) { // Over 10 scales
//		for(int iiChKernel = 0; iiChKernel < nKernels; iiChKernel++) {// Over 4 kernels
...
void PedestrianDetector::DeterminePyramidParameters(cv::Size2i imageSize) {
	...
	getScales(imagePyramid.nPerOct,imagePyramid.nOctUp,imagePyramid.minDs,imagePyramid.shrinkFirstStage,dimImage,scales,scaleshw);
...
void showDataFloat(string label,Mat image,int nDims, unsigned int selectRow){
	std::setprecision(4);
	...
	for (int nn = 0; nn < nDims;nn++)
	{
		...
		{
			//cout << std::setprecision(4) << newJ[n1+nRows*nCols*nn+selectRow*nRows] << ", ";
			printf(" %.4f, ", newJ[n1+nRows*nCols*nn+selectRow*nRows]);
		}
		cout << endl;
	}
}
...
Mat PedestrianDetector::convert_PD_2_OpenCV(Mat image,bool isChar) {
	...
	unsigned int nRows = image.rows;
	unsigned int nCols = image.cols;
	...
	for (int row = 0; row < image.rows;row++)
		...
		convert_PD_2_OpenCV_FAST((float*)image.data,(float*)remappedImage.data,nRows,nCols,image.channels());
//		for (int row = 0; row < image.rows;row++)
//			for(int col = 0; col < image.cols; col++)
//				for(int ch = image.channels()-1; ch >= 0; ch--)
//					remappedImage.at<float>(n++) = image.at<float>(row + col*image.rows + ch*image.rows*image.cols);

	}

	return remappedImage;
}
void PedestrianDetector::convert_PD_2_OpenCV_FAST(float* from, float* to,unsigned int nRows, unsigned int nCols, unsigned int channels) {
	unsigned int nRowsCols = nRows*nCols;
	unsigned int n = 0;
	for (unsigned int row = 0; row < nRows;row++)
		for(unsigned int col = 0; col < nCols; col++)
			for(int ch = channels-1; ch >= 0; ch--)
				to[n++] = from[row + col*nRows + ch*nRowsCols];
}



Mat PedestrianDetector::convert_OpenCV_2_PD(Mat inputImage, bool isChar) {
		...
		for (unsigned int row = 0; row < nRows;row++)
			for(unsigned int col = 0; col < nCols; col++)
				for(int ch = inputImage.channels()-1; ch >= 0; ch--)
					...
					//outputImage.at<uchar>(row + col*nRows + ch*nRowsCols) = inputImage.at<uchar>(n++);
	}
	else {
		convert_OpenCV_2_PD_FAST((float*)inputImage.data, (float*)outputImage.data,nRows, nCols,inputImage.channels());
//		for (unsigned int row = 0; row < nRows;row++)
//			for(unsigned int col = 0; col < nCols; col++)
//				for(int ch = inputImage.channels()-1; ch >= 0; ch--)
//					outputImage.at<float>(row + col*nRows + ch*nRowsCols) = inputImage.at<float>(n++);
...
void PedestrianDetector::convert_OpenCV_2_PD_FAST(float* from, float* to,unsigned int nRows, unsigned int nCols, unsigned int channels) {
	...
	for (unsigned int row = 0; row < nRows;row++)
		for(unsigned int col = 0; col < nCols; col++)
			for(int ch = channels-1; ch >= 0; ch--)
				to[row + col*nRows + ch*nRowsCols] = from[n++];
}
...
vector<bbType>  PedestrianDetector::pedDetector(cv::Mat inputImage){
	...
	clock_t start, end;
	vector<double> tConvert2Opencv;
	vector<double> tConvert2PD;
	...
	if(validMatFile) {
		...
		//showOpenCVData("Input image (OpenCV)", inputImage,true);

		start = clock();
		imageMATLAB = convert_OpenCV_2_PD(inputImage, true);
		end = clock();
		tConvert2PD.push_back(end-start);
		//showDataChar("Input image(PD):",imageMATLAB,imageMATLAB.channels()); // Show result of resampled image.
		...
		for (unsigned int n = 0; n < isRealPyramid.size(); n++)
		{
			...
			//Size2i sz1;
			sz1.width =round(dimImage.width*s/imagePyramid.shrinkFirstStage)*imagePyramid.shrinkFirstStage;
			sz1.height =round(dimImage.height*s/imagePyramid.shrinkFirstStage)*imagePyramid.shrinkFirstStage;
			...
			//Size2i cr;
			//Size2i shrinkedSize;
			cr.height = resampledImage2.size[0] % int(imagePyramid.shrinkFirstStage);
			cr.width = resampledImage2.size[1] % int(imagePyramid.shrinkFirstStage) ;
			...
			shrinkedSize.height=shrinkedSize.height/imagePyramid.shrinkFirstStage;
			shrinkedSize.width=shrinkedSize.width/imagePyramid.shrinkFirstStage;
...
//					showDataFloat("Final Color Channels",allChannels[nn][0],allChannels[nn][0].channels());
...
//					showDataFloat("Result of Histogram",allChannels[nn][2], 6); // Show result of filtered image.
		...
		for(unsigned int n = 0; n<isApproxPyramid.size();n++){
			...
			float s = scales.at<float>(nn,0);
			Size2i sz1;
			sz1.width =round(dimImage.width*s/imagePyramid.shrinkFirstStage);
			sz1.height =round(dimImage.height*s/imagePyramid.shrinkFirstStage);
			for(int nnn = 0; nnn < 3; nnn++){

				double ratio = pow(s/scales.at<float>(isReal,0),-imagePyramid.lambdas[nnn]);
				//cout << "n: " << nnn << endl;
				//cout << "lambda: " << imagePyramid.lambdas[nnn] << endl;
		...
		Size2i pad2(imagePyramid.pad.width/imagePyramid.shrinkFirstStage,imagePyramid.pad.height/imagePyramid.shrinkFirstStage);
		...
		int nKernels = 4;
		Mat allChannelsPadFiltered[nScales][pedModel.nChannels*nKernels];
		if(fastDetector == 0) {
			//imagePyramid.shrink = imagePyramid.shrink*2;
			...
			for(int iScales = 0; iScales<nScales;iScales++) {
				...
				for(int iiTopChannels = 0; iiTopChannels < 3; iiTopChannels++) {
					...
					//showDataFloat("Plotting single channels1: ", allChannelsPad[iScales][iiTopChannels],allChannelsPad[iScales][iiTopChannels].channels());
					start = clock();
					Mat tmpMat = convert_PD_2_OpenCV(allChannelsPad[iScales][iiTopChannels],0);//,to);
					end = clock();
					tConvert2Opencv.push_back(end-start);
					//Mat tmpMat = convert_PD_2_OpenCV2(allChannelsPad[iScales][iiTopChannels],0,to);
					//showOpenCVData("Plotting single channels2: ", tmpMat,false);
					split(tmpMat,singleChannels);

					int nSingleChannels = singleChannels.size();


//					for(int iiiChannels = nSingleChannels-1; iiiChannels >= 0; iiiChannels--){
//						cout << iiiChannels << ", ";
//						showOpenCVData("Plotting single channels3: ", singleChannels[iiiChannels],false);
//					}
					...
					for(int iiiChannels = nSingleChannels-1; iiiChannels >= 0; iiiChannels--){
						for(int iiiiFilters = 0; iiiiFilters < nKernels; iiiiFilters++) {
							...
							//cout << "iiiChannels: " << iiiChannels;
							//cout << ", iiiiFilters: " << iiiiFilters << ", channelCountTo10: " << channelCountTo10 << ", Store index: " << channelCountTo10+iiiiFilters*10 << endl;
							Mat tmpOutput;

							Mat tmpKernel = pedModel.kernel[channelCountTo10][iiiiFilters];
							Point anchor(tmpKernel.cols - tmpKernel.cols/2 -1, tmpKernel.rows - tmpKernel.rows/2 -1);
							int borderMode = BORDER_CONSTANT;

							filter2D(singleChannels[iiiChannels],tmpOutput,singleChannels[iiiChannels].depth(),tmpKernel,anchor, 0,borderMode);
							//namedWindow("BoundingsBox2", CV_WINDOW_AUTOSIZE);
							//imshow("BoundingsBox2",tmpOutput);

							//showOpenCVData("Test", singleChannels[iiiChannels],false);
							//cout << tmpKernel << endl;

							//showOpenCVData("Conv output (Opencv)", tmpOutput,false);
							//tmpOutput = convert_OpenCV_2_PD(tmpOutput,false);
							//showDataFloat("Conv output (PD)", tmpOutput,tmpOutput.channels());
							start = clock();
							tmpOutput = convert_OpenCV_2_PD(tmpOutput,false);
							end = clock();
							tConvert2PD.push_back(end-start);

							allChannelsPadFiltered[iScales][channelCountTo10+iiiiFilters*pedModel.nChannels] = pdImResample(tmpOutput,int(round(float(tmpOutput.rows)/2)), int(round(float(tmpOutput.cols)/2)),1.0);
							//cout << channelCountTo10+iiiiFilters*10 << " ";
							//showDataFloat("Resampled", tmpOutput2,tmpOutput2.channels());

							//allChannelsPadFiltered[iScales][channelCountTo10+iiiiFilters*pedModel.nChannels] = tmpOutput2;
							//cout << "test: " << endl;
							//showDataFloat("Extreme test", allChannelsPadFiltered[iScales][channelCountTo40],allChannelsPadFiltered[iScales][channelCountTo40].channels());
						}
						channelCountTo10++;
					}
				}
...
//				for(int n = 0; n < 40; n++) {
//					cout << n << "; ";
//					showDataFloat("Channel",allChannelsPadFiltered[iScales][n],1);
//				}
			}
		}
		...
		for(int iScales = 0; iScales<nScales;iScales++)
		{
			...
			int nChannels;
			if(fastDetector == 1) {
				nChannels = pedModel.nChannels;
				nRows = allChannelsPad[iScales][0].size[0];
				nCols = allChannelsPad[iScales][0].size[1];
			}
			else {
				nChannels = pedModel.nChannels*4;
				nRows = allChannelsPadFiltered[iScales][0].size[0];
				nCols = allChannelsPadFiltered[iScales][0].size[1];
			}

			int nChannelPixels = nRows*nCols;
			Mat channelScalei(nRows, nCols,CV_32FC(nChannels));
				...
				for (int iiChannels = 0; iiChannels < 3;iiChannels++)
					for (int iiiSubChannel= 0; iiiSubChannel < allChannelsPad[iScales][iiChannels].channels(); iiiSubChannel++)
						for (int iiiiPixels= 0; iiiiPixels < nChannelPixels; iiiiPixels++)
							channelScalei.at<float>(cData++) = allChannelsPad[iScales][iiChannels].at<float>(iiiiPixels+nChannelPixels*iiiSubChannel);
			}
			else {
				for (int iChannels = 0; iChannels < nChannels;iChannels++)
					for (int iiiiPixels= 0; iiiiPixels < nChannelPixels; iiiiPixels++)
						channelScalei.at<float>(cData++) = allChannelsPadFiltered[iScales][iChannels].at<float>(iiiiPixels);
			}
			//printf("Scale: %i\n",iScales);
			//showDataFloat("",channelScalei,channelScalei.channels(),4); // Show result of filtered image.

			// Detection is performed.
			vector<bbType> bbsTmp = pfDetect(channelScalei, imagePyramid.shrinkLastStage, pedOptions.modelDsPad.height, pedOptions.modelDsPad.width, pedOptions.stride,pedOptions.cascThr,channelScalei.rows,channelScalei.cols,channelScalei.channels(), pedModel);
			bbType bbsTmpReal;
...
//				cout << bbsTmp[iBbs].x1 << ", " << bbsTmp[iBbs].y2 << ", " << bbsTmp[iBbs].width3 << ", " << bbsTmp[iBbs].height4 << ", " << bbsTmp[iBbs].score5 << endl;
//				cout << bbsTmpReal.x1 << ", " << bbsTmpReal.y2 << ", " << bbsTmpReal.width3 << ", " << bbsTmpReal.height4 << ", " << bbsTmpReal.score5 << endl;
				bbs.push_back(bbsTmpReal);
			}
		}
//		std::sort(bbs.begin(), bbs.end(), by_score());
//		for(unsigned int iBbs = 0; iBbs < bbs.size(); iBbs++)
//		{
//			cout << bbs[iBbs].x1 << ", " << bbs[iBbs].y2 << ", " << bbs[iBbs].width3 << ", " << bbs[iBbs].height4 << ", " << bbs[iBbs].score5 << endl;
//		}
		...
		double sum1  = 0;
		for(int n = 1; n < tConvert2Opencv.size();n++) {
			sum1 = sum1+tConvert2Opencv[n];
		}
		double time1 = (double) (sum1) / CLOCKS_PER_SEC * 1000.0;
		double sum2  = 0;
		for(int n = 1; n < tConvert2PD.size();n++) {
			sum2 = sum2+tConvert2PD[n];
		}
		double time2 = (double) (sum2) / CLOCKS_PER_SEC * 1000.0;
		cout << "Convert to OpenCV: " << time1 << " ms" << endl;
		cout << "Convert to Piotr Dollar: " << time2 << " ms" << endl;
			...
			if(fieldNumber1 == detOpt) {
				...
				// Load all variables from Opt.pPyramid.
					...
					// Load variable from Opt.pPyramid.lambdas
						...
						imagePyramid.lambdas.insert(imagePyramid.lambdas.end(),&doubleVector[0],&doubleVector[matVarField3->dims[0]*matVarField3->dims[1]]); // Three values only.
					...
					// Load variable from Opt.pPyramid.pChns.
						...
						if(fieldNumber4 == detOptPyramidChns_shrink) {
							imagePyramid.shrinkFirstStage = *(double*) (matVarField4->data);
						}
					}
				}
			}
